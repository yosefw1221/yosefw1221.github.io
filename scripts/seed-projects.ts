...
async function seedProjects() {
  try {
    ...
    const existingTitles = snapshot.docs.map((doc: any) => doc.data().title);

    console.log(`Found ${snapshot.size} existing projects.`);

    // Add only new projects (skip duplicates by title)
    let addedCount = 0;
    ...
    if (addedCount > 0) {
      console.log(`🎉 Successfully added ${addedCount} new project(s)!`);
    } else {
      console.log('✨ All projects already exist. Nothing to add.');
    }
  } catch (error) {
