            ...
            return (
              ...
              >
                ...
                <motion.div
                  ...
                  className={`md:w-[calc(50%-20px)] ${isEven ? 'md:mr-auto' : 'md:ml-[calc(50%+20px)]'}`}
                >
                  <div className="theme-card rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                    ...
                    <div className="p-6 border-b theme-border">
                      <div className="flex flex-col gap-4">
                        <div>
                          <h3 className="text-xl sm:text-2xl font-bold text-blue-500">{experience.company}</h3>
                          <h4 className="text-lg sm:text-xl font-medium theme-text-primary mt-1">{experience.title}</h4>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-3">
                          <div className="flex items-center gap-1.5 theme-text-secondary text-sm border theme-border px-3 py-1.5 rounded-full">
                            ...
                            {experience.duration && (
                              <span className="ml-1 text-blue-500 font-medium">({experience.duration})</span>
                            )}
                          </div>
                    ...
                    <div className="p-6">
                      ...
                      <div className="theme-text-secondary text-sm sm:text-base leading-relaxed">
                        {experience.description && (
                          typeof experience.description === 'string' && experience.description.includes('<') ? (
                            <div dangerouslySetInnerHTML={{ __html: experience.description }} className="prose prose-sm max-w-none theme-text-primary" />
                          ) : (
                            <ul className="list-disc pl-5 space-y-2">
                              {(typeof experience.description === 'string' ? 
                                experience.description.toString().split('.').filter(s => s.trim().length > 0) : 
                                [experience.description]
                              ).map((sentence, i) => (
                                <li key={i} className="transition-colors duration-300 hover:text-blue-500">
                                  {typeof sentence === 'string' ? sentence.trim() + (sentence.endsWith('.') ? '' : '.') : sentence}
                                </li>
                              ))}
                            </ul>
                          )
